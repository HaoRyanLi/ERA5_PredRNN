...
dataset_path = '/scratch/09012/haoli1/ERA5/dataset_5_vars/'
infile = sys.argv[1]
...
for curr_year in years:
    ...
    final_data = np.stack([stack_u_wind, stack_v_wind, stack_press, stack_precip, stack_temp], axis=1)
    ...
    final_ds['dims'] = np.array([[final_data.shape[1],720,1440]]).astype(np.int32)
    final_ds['clips'] = final_clips.astype(np.int32)

    np.savez(dataset_path+f'era5_train_{str(curr_month).zfill(2)}01{str(curr_year)}_{final_data.shape[0]}_24hr.npz', **final_ds)